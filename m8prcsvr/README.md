...
1. Create a custom sensors directory in your user space:
   ...
   ```bash
   mkdir -p ~/instana-plugins/custom_sensors/microstrategy_m8prcsvr
...
2. Copy the necessary files:
   ...
   ```bash
   cp -r m8prcsvr/* ~/instana-plugins/custom_sensors/microstrategy_m8prcsvr/
...
4. Set up a user-level service or cron job to run the sensor:
   ...
   * * * * * env PYTHONPATH=/home/yourusername/instana-plugins/custom_sensors /home/yourusername/instana-plugins/custom_sensors/microstrategy_m8prcsvr/sensor.py
   ```
...
1. Ensure your user has permission to read `/proc` entries for the M8PrcSvr processes
2. If the M8PrcSvr processes run as a different user, you may need to:
...
2. Configure the sensor to use TLS by setting environment variables:
   ...
   /opt/instana/agent/plugins/custom_sensors/microstrategy_m8prcsvr/sensor.py
   ```
...
- `common/process_monitor.py` to collect metrics about M8PrcSvr processes
...
```bash
/opt/instana/agent/plugins/custom_sensors/microstrategy_m8prcsvr/sensor.py --run-once
```
...
1. Verify the M8PrcSvr process is running: `ps aux | grep -i m8prcsvr`
2. Check if the sensor is running: `ps aux | grep microstrategy_m8prcsvr`
...
5. **Log File Location**:
   ...
   - Specify a custom log file with `--log-file`:
     ...
     ```bash
     ./sensor.py --log-file=/var/log/instana/m8prcsvr.log
     ```

### Edge Cases and Limitations

1. **Multiple M8PrcSvr Instances**:
